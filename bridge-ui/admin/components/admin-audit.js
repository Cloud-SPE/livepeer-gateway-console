  ...
  render() {
    return html`
      <header class="card-header">
        ...
        <button
          type="button"
          @click=${() => void this._loadFirstPage()}
      ...
      <p class="muted">
        Bearer-action log. Newest first; click "Older" to page back.
      </p>
      ...
      ${this._events.length === 0
        ? html`<p class="muted">
            ${this._loading ? "Loading…" : "No events yet."}
          </p>`
        : html`
            <table class="data">
              <thead>
                ...
                ${this._events.map(
                  (e) => html`
                    <tr class=${e.ok ? "" : "row-fail"}>
                      ...
                      <td>${e.action}</td>
                      <td>
                        ${e.target ? html`<code>${e.target}</code>` : "—"}
                      </td>
                      <td>${e.ok ? "yes" : "no"}</td>
                      <td>${e.message ?? "—"}</td>
                    </tr>
