...
function knownEntry(address: string, freshness: KnownOrch['freshnessStatus']): KnownOrch {
  return { address, mode: 'well-known', freshnessStatus: freshness, cachedAt: 1_700_000_000_000 };
}
...
function makeChain(opts: FakeChainOptions): FakeChain {
  ...
  const spies = {
    ...
    readServiceUri: vi.fn(async (_sr: Address, addr: Address) => serviceUris[addr.toLowerCase()] ?? ''),
  };
  return {
    chainId: 42_161,
