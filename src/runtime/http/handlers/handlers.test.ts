...
function makeReq(o: { actor?: string; query?: unknown; params?: unknown; body?: unknown } = {}) {
  return {
...
describe('handleListOrchs', () => {
  ...
  it('client-side filters by capability + model', async () => {
    const listOrchs = vi.fn(async () => [
      rosterRow(ORCH_A, ['whisper'], ['whisper-large']),
      rosterRow('0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', ['transcode'], []),
    ]);
    const reply = makeReply();
    await handleListOrchs(
      makeReq({ query: { capability: 'whisper', model: 'whisper-large' } }) as never,
      reply as never,
      { routing: makeRouting({ listOrchs }) },
    );
...
describe('handleGetOrch', () => {
  ...
  it('returns 404 when both orch and observations are empty', async () => {
    ...
    expect(reply.body).toMatchObject({ error: { code: 'orch_not_found', address: ORCH_A } });
  });
...
describe('handleCapabilitySearch', () => {
  it('forwards capability/model/tier to resolver.search', async () => {
    ...
    await handleCapabilitySearch(
      makeReq({ query: { capability: 'whisper', model: 'whisper-large', tier: 'tier-0' } }) as never,
      reply as never,
  ...
  it('omits undefined optional fields', async () => {
    const search = vi.fn(async () => ({ orchAddress: null, reason: 'no node matched', nodes: [] }));
    const reply = makeReply();
    await handleCapabilitySearch(
      makeReq({ query: { capability: 'whisper' } }) as never,
      reply as never,
...
describe('handleGetSenderWallet', () => {
  ...
  it('maps SenderWalletNotConfiguredError to 503 wallet_not_configured', async () => {
    ...
    expect(reply.body).toMatchObject({ error: { code: 'wallet_not_configured' } });
  });
...
describe('handleGetSenderEscrow', () => {
  it('returns the escrow snapshot', async () => {
    const getEscrow = vi.fn(async () => ({ depositWei: '1', reserveWei: '2', observedAt: 9 }));
    const reply = makeReply();
    await handleGetSenderEscrow(
      makeReq() as never,
      reply as never,
...
describe('handleHealth', () => {
  it('200 when both daemons ping ok', async () => {
    ...
    await handleHealth(
      ...
      reply as never,
      {
        ...
        payer: { ping: async () => ({ ok: true }), getDepositInfo: async () => ({ depositWei: '0', reserveWei: '0', withdrawRound: '0' }), close: () => undefined },
  ...
  it('503 when one daemon reports unavailable', async () => {
    ...
    await handleHealth(
      ...
      reply as never,
      {
        resolver: makeResolverClient({ ping: async () => ({ ok: false, error: 'down' }) }),
        payer: { ping: async () => ({ ok: true }), getDepositInfo: async () => ({ depositWei: '0', reserveWei: '0', withdrawRound: '0' }), close: () => undefined },
        resolverSocketPath: '/tmp/r.sock',
        senderSocketPath: '/tmp/s.sock',
      },
...
function makeResolverService(overrides: Partial<ResolverService>): ResolverService {
  return {
...
function makeSender(overrides: Partial<SenderService>): SenderService {
  return {
    getWallet: async () => ({ address: '0xc', balanceWei: '0', minBalanceWei: null }),
    getEscrow: async () => ({ depositWei: '0', reserveWei: '0', observedAt: 0 }),
    ...overrides,
  };
}

function makeResolverClient(overrides: Partial<ResolverClient>): ResolverClient {
  return {
    ping: async () => ({ ok: true }),
    listKnown: async () => [],
