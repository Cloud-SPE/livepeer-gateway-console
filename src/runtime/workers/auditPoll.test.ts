...
describe("auditPoll worker", () => {
  it("first tick inserts events and advances the watermark", async () => {
    ...
    const obsA = await routingObservationsRepo.listRecentForOrch(
      sqlite.db,
      ORCH_A,
    );
  ...
  it("skips events with no orch address", async () => {
    ...
    const all = await routingObservationsRepo.listRecentForOrch(
      sqlite.db,
      ORCH_A,
    );
...
function auditEntry(
  overrides: Partial<ResolverAuditEntry>,
): ResolverAuditEntry {
  return {
    occurredAt: 1_700_000_000_000,
    orchAddress: ORCH_A,
